...
'quiz':{
'基礎卷':[
 ('遺傳學之父是？','孟德爾','遺傳學之父：孟德爾用豌豆做雜交實驗，發現了遺傳的基本規律，被稱為遺傳學之父'),
 ('孟德爾用什麼做實驗？','豌豆','豌豆實驗：孟德爾選用性狀分明又容易栽培的豌豆，進行大量雜交實驗找出遺傳規律'),
 ('會表現出來的性狀是？','顯性','顯性表現：兩個不同基因同時存在時，會表現出來讓人看得到的性狀稱為顯性'),
 ('被遮蓋的性狀是？','隱性','隱性遮蓋：和顯性基因同時存在時會被遮蓋、不會表現出來的性狀稱為隱性'),
 ('基因成對還單獨存在？','成對','成對存在：控制性狀的基因在體細胞中是成對存在的，一個來自父方一個來自母方'),
 ('生殖細胞形成時基因會？','分開','基因分開：形成生殖細胞時，成對的基因會分開，各自進入不同的生殖細胞'),
 ('顯性基因常用什麼表示？','大寫字母','符號表示：習慣上用大寫字母(如A)來代表顯性基因，方便寫出遺傳圖解'),
 ('隱性基因常用什麼表示？','小寫字母','符號表示：習慣上用小寫字母(如a)來代表隱性基因，方便寫出遺傳圖解'),
],
'進階卷':[
 ('Aa表現顯性還隱性？','顯性','顯性表現：只要基因型中含有一個顯性基因A，個體外觀就會表現出顯性性狀'),
 ('aa表現顯性還隱性？','隱性','隱性表現：基因型兩個都是隱性a，沒有顯性基因可以遮蓋，才會表現出隱性性狀'),
 ('Aa×Aa子代顯隱性比約？','3:1','孟德爾比例：Aa和Aa交配後，子代表現型顯性與隱性的比例大約是3比1'),
 ('分離律指成對基因？','分開進生殖細胞','分離律：成對的基因在形成生殖細胞時會彼此分開，各自進入不同的生殖細胞中'),
 ('AA與Aa外表(性狀)？','都顯性','外表相同：AA和Aa雖然基因型不同，但因為都含有顯性基因A，外表性狀都是顯性'),
 ('隱性性狀出現需基因型？','aa','需要兩個隱性：個體必須兩個基因都是隱性a，也就是基因型aa，隱性性狀才會表現出來'),
 ('生殖細胞各帶幾個某基因？','一個','各帶一個：成對基因在減數分裂時分開，所以每個生殖細胞只會帶到其中一個基因'),
 ('遺傳圖解可推算什麼？','子代性狀比例','推算比例：畫出遺傳圖解可以幫助我們推算出子代各種基因型與性狀出現的比例'),
],
'挑戰卷':[
 ('Aa×Aa子代基因型與比例？','AA:Aa:aa=1:2:1','基因型比例：Aa與Aa交配後，子代基因型AA、Aa、aa出現的比例大約是1比2比1'),
 ('為何隱性性狀可能隔代出現？','隱性基因被攜帶未表現後代aa才顯現','隔代出現：父母雖表現顯性卻可能都帶有隱性基因(Aa)，子代若剛好得到兩個隱性基因aa，隱性性狀就會重新出現'),
 ('用分離律解釋生殖細胞基因分配？','成對基因分開各生殖細胞帶一個','分離律解釋：體細胞中成對的基因在形成生殖細胞時會分開，讓每個生殖細胞只帶其中一個基因'),
 ('父母皆顯性卻生出隱性子代說明？','父母皆為Aa帶隱性基因','推知基因型：父母外表都是顯性卻生出隱性子代，代表雙方其實都是Aa，暗藏著一個隱性基因'),
 ('AA×aa子代基因型與性狀？','全Aa顯性','固定結果：純種顯性AA與純種隱性aa交配，子代基因型必定全部是Aa，外表全部表現顯性'),
 ('顯性與隱性的正確理解？','指是否表現非好壞','觀念釐清：顯性和隱性只是說明基因會不會表現出來，並不代表哪一種性狀比較優秀或比較差'),
 ('如何由子代比例反推親代基因型？','3:1推親代皆Aa','反推親代：若觀察到子代顯隱性比例接近3比1，就可以推測親代雙方基因型都是Aa'),
 ('孟德爾實驗對遺傳學的意義？','建立遺傳基本規律','奠定基礎：孟德爾透過豌豆實驗歸納出的分離律等規律，奠定了現代遺傳學的重要基礎'),
],
},
}
...
{'html':'''<div class="eyebrow">基因型 vs 表現型</div><h2>三種基因型</h2><table class="tbl"><tr><th>基因型</th><th>表現型</th><th>稱呼</th></tr><tr><td>AA</td><td>顯性</td><td>純種顯性</td></tr><tr><td>Aa</td><td>顯性</td><td>雜合(帶因)</td></tr><tr><td>aa</td><td>隱性</td><td>純種隱性</td></tr></table><div class="box y">AA 與 Aa 外表都是顯性，但 Aa 偷偷帶著隱性基因。</div>'''},
{'html':'''<div class="eyebrow">例題 ★☆☆</div><h2>先熱身</h2><div class="eg"><div class="q">(1) aa 表現顯性還是隱性？ (2) Aa 表現？</div></div><div class="box y"><p style="margin:0;">(1) aa → <span class="hl-r">隱性</span>(沒有顯性基因)</p><p style="margin:0;">(2) Aa → <span class="hl">顯性</span>(有一個A就顯性)</p></div>'''},
{'html':'''<div class="eyebrow">核心 ④</div><h2>遺傳圖解：Aa × Aa</h2><div class="big-center"><svg viewBox="0 0 220 200" width="52%"><text x="90" y="20" font-size="14" fill="#f0d878">父 Aa</text><text x="-115" y="16" font-size="14" fill="#f0d878" transform="rotate(-90)">母 Aa</text><line x1="60" y1="30" x2="60" y2="190" stroke="#f4f1e8" stroke-width="1"/><line x1="140" y1="30" x2="140" y2="190" stroke="#f4f1e8" stroke-width="1"/><line x1="20" y1="70" x2="200" y2="70" stroke="#f4f1e8" stroke-width="1"/><line x1="20" y1="130" x2="200" y2="130" stroke="#f4f1e8" stroke-width="1"/><text x="90" y="55" font-size="15" fill="#9fc8d8">A</text><text x="170" y="55" font-size="15" fill="#9fc8d8">a</text><text x="35" y="105" font-size="15" fill="#e8a0a0">A</text><text x="35" y="165" font-size="15" fill="#e8a0a0">a</text><text x="82" y="105" font-size="16" fill="#f4f1e8">AA</text><text x="162" y="105" font-size="16" fill="#f0d878">Aa</text><text x="82" y="165" font-size="16" fill="#f0d878">Aa</text><text x="162" y="165" font-size="16" fill="#e8a0a0">aa</text></svg></div><div class="box">基因型 AA : Aa : aa = <span class="hl">1 : 2 : 1</span></div>'''},
{'html':'''<div class="eyebrow">解讀圖解</div><h2>子代比例</h2><ul><li>基因型 AA : Aa : aa = <span class="hl">1 : 2 : 1</span></li><li>表現型 顯性 : 隱性 = <span class="hl-b">3 : 1</span>(AA、Aa、Aa 都顯性)</li></ul><div class="box y">「3:1」是孟德爾最著名的比例。</div>'''},
{'html':'''<div class="eyebrow">例題 ★★☆</div><h2>試試看</h2><div class="eg"><div class="q">純種顯性 AA × 純種隱性 aa，子代基因型與表現型？</div></div><div class="box y"><p style="margin:0;">子代全部 <span class="hl">Aa</span></p><p style="margin:6px 0 0;">表現型：全部<span class="hl">顯性</span></p></div>'''},
{'html':'''<div class="eyebrow">易錯釐清</div><h2>帶因者的祕密</h2><div class="box r">父母外表都是顯性，卻生出隱性(aa)的孩子？</div><ul><li>因為父母其實都是 <span class="hl">Aa</span>(帶因者)</li><li>各給一個 a → 子代 aa 表現隱性</li></ul><div class="box y">隱性性狀可能「隔代」出現，就是這個原因。</div>'''},
{'html':'''<div class="eyebrow">例題 ★★★</div><h2>反推親代</h2><div class="eg"><div class="q">某對親代子代出現顯性:隱性≈3:1，親代基因型最可能是？</div></div><div class="box y">3:1 是 <span class="hl">Aa × Aa</span> 的特徵比例 → 父母都是雜合帶因者。</div>'''},
{'html':'''<div class="eyebrow">素養 · 生活</div><h2>遺傳就在你身邊</h2><ul><li><span class="hl-b">捲舌</span>與否、有無<span class="hl-b">酒窩</span>、耳垂貼不貼</li><li>親子相似，正是基因一代代傳遞</li></ul><div class="box y">觀察家人的性狀，就是一場迷你遺傳學實驗。</div>'''},
{'html':'''<div class="eyebrow">本節總結</div><h2>三句話記住</h2><div class="box"><span class="hl">顯隱性：</span>顯性遮蓋隱性，A大a小</div><div class="box-b"><span class="hl-b">分離律：</span>成對基因分開進生殖細胞</div><div class="box r"><span class="hl-r">比例：</span>Aa×Aa → 基因型1:2:1、表現型3:1</div>'''},
