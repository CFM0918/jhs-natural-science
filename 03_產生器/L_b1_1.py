...
'quiz':{
'基礎卷':[
 ('科學方法通常從哪一步開始？','觀察','觀察現象才引發問題：科學探究通常先觀察到某個現象，才會進一步引發想要探究的問題'),
 ('SI 制中長度的單位是？','公尺(m)','國際基本量：國際單位制規定長度的基本單位是公尺，符號寫作m'),
 ('SI 制中質量的單位是？','公斤(kg)','基本量：國際單位制規定質量的基本單位是公斤，符號寫作kg'),
 ('SI 制中時間的單位是？','秒(s)','基本量：國際單位制規定時間的基本單位是秒，符號寫作s'),
 ('實驗中刻意改變的變因稱為？','操縱變因','即自變項：實驗中由研究者刻意改變的那個變因，稱為操縱變因，又稱自變項'),
 ('隨之改變、被測量的變因稱為？','應變變因','即依變項：隨著操縱變因改變而跟著變化、需要被測量記錄的結果，稱為應變變因'),
 ('為求公平而保持不變的變因？','控制變因','其他條件相同：除了操縱變因以外，其他必須保持相同以確保比較公平的變因，稱為控制變因'),
 ('對照組的作用是什麼？','作為比較的基準','凸顯操縱變因效應：對照組沒有施加操縱變因，作為比較的基準，才能凸顯出操縱變因真正造成的效應'),
 ('讀尺時要估到最小刻度的？','下一位','估計值：測量讀數時要在最小刻度之下再合理估讀一位，這一位稱為估計值'),
 ('溫度的 SI 基本單位是？','克耳文(K)','絕對溫標：國際單位制規定溫度的基本單位是克耳文，是一種絕對溫標'),
],
'進階卷':[
 ('研究「陽光影響植物生長」，操縱變因是？','陽光(照光量)','刻意改變的量：這個實驗中研究者刻意改變的是陽光的照射量，所以陽光就是操縱變因'),
 ('承上，應變變因是？','植物生長高度','被測量的結果：植物長高的程度是隨陽光多寡而改變、需要測量記錄的結果，所以是應變變因'),
 ('承上，需控制的變因舉一例？','水量/土壤/溫度','保持相同：除了陽光之外，水量、土壤種類、溫度等都必須保持相同，才能確保只有陽光造成生長差異'),
 ('最小刻度 0.1cm 的尺，長度應記錄到哪一位？','0.01cm 位','估到下一位：讀數時要在最小刻度0.1cm之下再估讀一位，也就是要記錄到0.01cm這一位'),
 ('3.50cm 有幾位有效數字？','3 位','末位 0 有效：3、5、0這三個數字都是測量得到的結果，末位的0也算有效數字，所以共有3位'),
 ('「糖加熱會焦化」在科學方法中屬於？','假設','對問題的暫時解答：這句話是還沒被驗證、對問題提出的暫時性解答，屬於假設'),
 ('只做一次實驗，結果可靠嗎？','不可靠','需重複多次：只做一次實驗容易受到偶然誤差影響，必須重複做多次才能提高結果的可靠度'),
 ('讀量筒體積時視線應對準？','凹液面最低處','避免視差：讀取量筒刻度時，視線要與凹液面的最低處保持水平，才能避免因視角不同造成的讀數誤差(視差)'),
 ('密度是基本量還是導出量？','導出量','質量÷體積：密度是由質量和體積這兩個基本量計算組合而成的，屬於導出量'),
 ('數據與假設不符時應如何？','如實記錄不竄改','誠實客觀：科學研究要求誠實記錄真實數據，即使與原本的假設不符也不能竄改，這是科學的基本態度'),
],
'挑戰卷':[
 ('設計「鹽量影響水沸點」實驗，寫出三類變因','操縱:鹽量 應變:沸點 控制:水量與火力','完整變因設計：這個實驗中，刻意改變的鹽量是操縱變因，被測量的沸點是應變變因，而水量、火力大小等則必須保持相同，屬於控制變因'),
 ('為何要設「不加鹽」的對照組？','作基準比較凸顯鹽的效應','對照精神：留一組不加鹽當作對照組，可以拿來和加鹽的各組比較，才能確定沸點的改變真的是鹽造成的'),
 ('甲量 2.5cm、乙量 2.50cm，誰較精密？','乙','估到更小一位：乙的讀數多了一位小數(0.50)，代表估讀到更小的位數，測量比甲更精密'),
 ('重複量同物 5 次取平均，目的是？','降低隨機誤差','提高可靠度：多次測量取平均值，可以抵消每次測量時偶然發生的隨機誤差，讓結果更接近真實值、更可靠'),
 ('12.0cm 與 12cm 意義相同嗎？','不同','前者精密到 0.1：12.0cm多寫了小數點後一位0，代表測量精密到0.1公分，和只寫12cm(精密到1公分)所代表的精密程度不同'),
 ('「這藥草能治百病」是科學主張嗎？','不是','無法檢驗、過度宣稱：科學主張必須能被具體驗證或否證，「治百病」這種空泛誇大、無法用實驗檢驗的說法不算科學主張'),
 ('面積是基本量或導出量？單位？','導出量, m²','長度×長度：面積是由長度這個基本量自己相乘組合出來的，屬於導出量，單位是m²'),
 ('數據異常過大時，最先該做什麼？','檢查步驟並重測','不可直接刪除：發現異常數據時，應該先檢查實驗步驟是否出錯、再重新測量確認，不能未經查證就直接刪除這筆數據'),
 ('1.5 km 等於幾公尺？','1500 m','×1000：1公里等於1000公尺，1.5乘以1000等於1500公尺'),
 ('科學結論為何常說「目前證據支持」？','科學具暫時性可修正','科學本質：科學結論是根據目前掌握的證據所得出的最佳解釋，未來若有新證據出現，結論隨時可能被修正，這正是科學具有暫時性的本質'),
],
},
'slides':[
